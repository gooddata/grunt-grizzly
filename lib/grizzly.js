...
Grizzly.prototype._createProxy = function() {
    ...
    const addCorsHeaders = function(req, res) {
        res.header('Access-Control-Allow-Origin', req.get('origin')); // allow-origin: '*' doesn't work... 
        res.header('Access-Control-Allow-Credentials', true);
        res.header('Access-Control-Allow-Methods', 'GET, POST');
        res.header('Access-Control-Allow-Headers', 'Content-Type, x-gdc-js-pkg, x-gdc-js-pkg-version, x-gdc-js-sdk-comp, x-gdc-js-sdk-comp-props');
    }
    
    app.options('*', function(req, res){
        addCorsHeaders(req, res);
        res.status(200).send('CORS preflight request OK.');
    });

    // everything else is proxied to the passed backend
    app.all('*', function(req, res){
        addCorsHeaders(req, res);
        proxySnippet(req, res);
    });
