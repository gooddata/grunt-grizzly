...
Grizzly.prototype.printStartedMessage = function() {
    ...
    console.error('Running grizzly server on ' + ('https://localhost:' + this._options.port).red);
...
Grizzly.prototype._createProxy = function() {
    ...
    app.use('/test', express.directory(this._options.root + '/test'));

    // Configure handling of static files in bear's document
    app.use(express.static(this._options.root));
